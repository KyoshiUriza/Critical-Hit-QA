...
// /QAHub/ subpath, is case-sensitive (Windows is not), and publishes only what
...
const BASE = (process.argv[2] || 'https://kyoshiuriza.github.io/QAHub').replace(/\/$/, '');
    ...
    const state = await page.evaluate(() => {
      ...
      return {
        ...
        escaped: [...document.querySelectorAll('.site-header nav a')]
          ...
          .filter((h) => h && h.startsWith('/') && !h.startsWith('/QAHub/')),
      };
    });
